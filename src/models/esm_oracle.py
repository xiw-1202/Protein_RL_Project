...
class ESM2Oracle:
    ...
    def __init__(self, model_name="esm2_t12_35M_UR50D", device="auto", cache_scores=True):
        """
        Initialize ESM-2 oracle
        
        Args:
            model_name: ESM-2 model variant
                - esm2_t12_35M_UR50D: Fast (recommended for testing)
                - esm2_t33_650M_UR50D: More accurate (recommended for experiments)
    ...
    def clear_cache(self):
        ...
        print(f"Cache cleared (had {len(self.score_cache)} entries)")


if __name__ == "__main__":
    # Quick test
    print("Testing ESM-2 Oracle...")
    print("="*70)
    ...
    test_seqs = [
        ...
        "MKTAYIAKQRQISFVKSHFSRQ",  # Duplicate (should use cache)
        "AKTAYIAKQRQISFVKSHFSRQ",  # Single mutation
    ]
    
    for i, seq in enumerate(test_seqs, 1):
        score = oracle.score_sequence(seq)
        print(f"{i}. Score: {score:.4f} (Queries: {oracle.get_query_count()})")
    
    print(f"\nCache hits: {3 - oracle.get_query_count()} / 3")
